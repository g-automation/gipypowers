...
This per-task loop plus the final whole-branch review is the skill's **two-stage review**: a task-scoped gate after every task, and one broad review after all tasks complete.

## Pre-Flight Plan Review

Before dispatching Task 1, scan the plan once for conflicts: tasks that contradict each other or the plan's Global Constraints, or anything the plan explicitly mandates that the review rubric treats as a defect (a test that asserts nothing, verbatim duplication of a logic block).
...
- **Report file:** name the implementer's report file after the brief (brief `…/task-N-brief.md` → report `…/task-N-report.md`) and put it in the dispatch prompt. The implementer writes the full report there and returns only status, commits, a one-line test summary, and concerns — **your final text IS the return value**: the controller never sees anything beyond what the subagent's last message says, so bulk detail belongs in the report file, not the message.
...
```
You: Using Subagent-Driven Development. [Read plan once, create todos for all tasks.]

Task 1: [task-brief for Task 1 → dispatch implementer with brief + report paths + context]
Implementer: asks a scoping question → you answer → implements, tests (5/5 passing),
  self-reviews (catches a missing flag, fixes it), commits.
[review-package → dispatch task reviewer]
Task reviewer: Spec ✅, no issues. Task quality: Approved. → mark Task 1 complete.

Task 2: [task-brief → dispatch implementer]
Implementer: no questions, implements, 8/8 tests passing, commits.
[review-package → dispatch task reviewer]
Task reviewer: Spec ❌ — missing progress reporting, extra unrequested flag,
  Important: magic number. [Dispatch fix subagent with all findings]
Fixer: removes the extra flag, adds progress reporting, extracts the constant.
Task reviewer (re-review): Spec ✅. Task quality: Approved. → mark Task 2 complete.

... [remaining tasks follow the same loop] ...

[After all tasks] Dispatch final code-reviewer → all requirements met, ready to merge. Done!
```

## Advantages

- **vs. manual execution:** subagents follow TDD naturally, fresh context per task (no confusion), subagent can ask questions before AND during work
- **vs. executing-plans:** same session (no handoff), continuous progress (no waiting), review checkpoints automatic
- **Efficiency:** controller curates exactly what context is needed — bulk artifacts move as files, not pasted text; subagent gets complete information upfront; questions surface before work begins
- **Quality gates:** self-review catches issues before handoff; task review carries two verdicts (spec compliance and code quality); review loops ensure fixes actually work
- **Cost:** more subagent invocations and review loops add iterations, but catch issues early — cheaper than debugging later
