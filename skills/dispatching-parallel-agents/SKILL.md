...
Issue all dispatches in the same response — that's what makes them run in parallel. Multiple dispatch calls in one response = parallel execution. One per response = sequential.
...
When agents return: read each summary, verify fixes don't conflict, run the full test suite, integrate all changes.
...
**Shared state:** Agents would interfere (editing same files, using same resources) — no shared state, no parallel dispatch
...
Result: all fixes independent, no conflicts, full suite green — 3 problems solved in the time of 1.
